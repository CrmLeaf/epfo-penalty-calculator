...
namespace Crmleaf\Payroll\Tools\EpfoPenaltyCalculator\Tests;

use Crmleaf\Payroll\Calculators\EpfoPenaltyCalculator;
use Crmleaf\Payroll\Tools\EpfoPenaltyCalculator\EpfoPenaltyCalculatorServiceProvider;
 ...
 */
final class EpfoPenaltyCalculatorTest extends TestCase
{
    /**
     * Held as a literal rather than as EpfoPenaltyCalculator::class so that a calculator
     * which does not exist fails as one readable assertion instead of a fatal
     * error that takes the rest of the file with it.
     */
    private const CALCULATOR = 'Crmleaf\Payroll\Calculators\EpfoPenaltyCalculator';
    ...
    {
        return [EpfoPenaltyCalculatorServiceProvider::class];
    }
    ...
    protected function defineEnvironment($app): void
    {
        ...
        // pins down.
        $app['config']->set('epfo-penalty-calculator.route.enabled', true);
    ...
    {
        $calculator = $this->app->make(EpfoPenaltyCalculator::class);

        self::assertInstanceOf(EpfoPenaltyCalculator::class, $calculator);
        self::assertSame($calculator, $this->app->make(EpfoPenaltyCalculator::class), 'The binding should be a singleton.');
    }

    public function test_the_configuration_is_merged(): void
    {
        self::assertSame('epfo-penalty-calculator', $this->app['config']->get('epfo-penalty-calculator.route.name'));
        self::assertSame('EPFO Penalty Calculator', $this->app['config']->get('epfo-penalty-calculator.view.title'));
    }

    public function test_the_route_is_off_until_it_is_asked_for(): void
    {
        /** @var array{route: array{enabled: bool}} $shipped */
        $shipped = require __DIR__.'/../config/epfo-penalty-calculator.php';
    ...
    {
        $html = Blade::render('<x-crmleaf::epfo-penalty-calculator />');

        self::assertStringContainsString('data-crmleaf-tool="epfo-penalty-calculator"', $html);
    ...
    {
        $response = $this->postJson('/tools/epfo-penalty-calculator', [
            'contribution_amount' => 250000,
            'wage_month' => '2025-01-01',
            'actual_payment_date' => '2025-05-20',
        ...
        $response->assertJsonPath('tool', 'epfo-penalty-calculator');
    }

    public function test_incomplete_input_is_rejected_rather_than_guessed_at(): void
    {
        $this->postJson('/tools/epfo-penalty-calculator', [])
            ->assertStatus(422)
