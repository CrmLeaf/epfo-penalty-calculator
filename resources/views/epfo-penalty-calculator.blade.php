...
<head>
    ...
    <title>{{ $title ?? 'EPFO Penalty Calculator' }}</title>
    <meta name="description" content="Interest under section 7Q and damages under 14B on delayed PF deposits.">
    <style>
...
<x-crmleaf::epfo-penalty-calculator
    ...
    :result="$result ?? null"
    :error="$error ?? null"
    :heading="$title ?? 'EPFO Penalty Calculator'"
/>
...
<footer class="crmleaf-tool__colophon">
    ...
    <p>
        Computed by <a href="https://github.com/crmleaf/epfo-penalty-calculator">crmleaf/epfo-penalty-calculator</a>,
...
{{--
    ...
    The bundled script binds any element carrying data-crmleaf-tool="epfo-penalty-calculator",
    computes on submit, and only falls back to posting the form if the browser
    build is not present.
--}}
@if (config('epfo-penalty-calculator.assets.script', true))
    @if (config('epfo-penalty-calculator.assets.cdn'))
        <script src="{{ config('epfo-penalty-calculator.assets.cdn') }}" defer></script>
    @endif
    <script src="{{ asset('vendor/epfo-penalty-calculator/epfo-penalty-calculator.js') }}" defer></script>
@endif
