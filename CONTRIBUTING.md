# Contributing to EPFO Penalty Calculator
...
```bash
git clone https://github.com/crmleaf/epfo-penalty-calculator.git
cd epfo-penalty-calculator
composer install
composer test
```
...
requires. `Crmleaf\Payroll\Calculators\EpfoPenaltyCalculator` is the class doing the work; everything else
...
Made in the monorepo, against `tools/epfo-penalty-calculator/`:
...
```
fix(epfoPenalty): <what changed>
test(epfoPenalty): <the case now covered>
```
